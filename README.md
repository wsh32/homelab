...
# Add Infisical + Vaultwarden credentials to terraform.tfvars
# Pass 2 — generates all service passwords, populates Vaultwarden automatically
cd terraform/nuc && terraform apply
cd terraform/anton && terraform apply

# Seed developer API keys into Infisical UI (Claude, Codex, GitHub, etc.)
```
...
|-------|------|---------------|
| **`terraform.tfvars`** | Infrastructure credentials (Proxmox, Tailscale, SSH key, ACME email) | Manually — 9 values total |
| **Vaultwarden** | All service passwords (Grafana, Jellyfin, n8n, etc.) | Automatically by `maxlaverse/bitwarden` Terraform provider during pass 2 |
| **Infisical** | Developer API keys (Claude, Codex, GitHub, etc.) | Manually via Infisical UI after bootstrap |

Service passwords are generated by Terraform (`random_password`) — never manually typed.
Containers get passwords via `.env` files written at provisioning time; no runtime dependency
on any secrets manager.
