...
| Node | Role |
|------|------|
| **Machamp** | Proxmox compute node. Hosts services VM (Jellyfin, Servarr, etc.) and dev workstation |
| **Diglett** | Always-on Proxmox infrastructure node. Hosts DNS (AdGuard Home), Tailscale coordination (Headscale), Traefik reverse proxy, step-ca local CA, Infisical, Vaultwarden, Authentik, and Home Assistant |
...
services/
  ...
  diglett-infra/         # Traefik + step-ca + Infisical + Vaultwarden + Authentik + Litestream
...
# Deploy diglett-infra (Traefik, Infisical, Vaultwarden, Authentik) and bootstrap Infisical
...
Infisical bootstrap secrets live at `/etc/homelab.env` on `diglett-infra` (root-owned, mode 0600), generated once by the `infra` Ansible role and NFS-persisted at `/mnt/nas/docker/infisical-backups/.secrets.env` for rebuild safety.
