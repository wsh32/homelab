...
- [ ] **Add Cloudflare provider to Terraform** — add `cloudflare/cloudflare` provider to `terraform/nuc/versions.tf`; add `cloudflare_api_token` to `variables.tf` and `terraform.tfvars.example`; add `cloudflare_tunnel` + `cloudflare_tunnel_config` + `cloudflare_record` resources for Headscale
- [ ] **Switch Terraform to S3 backend** — update `terraform/nuc/versions.tf` and `terraform/anton/versions.tf` to use an `s3` backend block pointed at MinIO on Storinator (`http://192.168.0.4:9000`, bucket `terraform-state`); add `minio_access_key` / `minio_secret_key` to `variables.tf` and `terraform.tfvars.example`
