...
- [ ] **Verify bridge_port values** — SSH to Anton and Redstone, run `ip link show`, confirm the NIC bridged to `vmbr0` is `eno1` (or correct it in `network.yml`)
- [ ] **Add Cloudflare provider to Terraform** — add `cloudflare/cloudflare` provider to `terraform/redstone/versions.tf`; add `cloudflare_api_token` to `variables.tf` and `terraform.tfvars.example`; add `cloudflare_tunnel` + `cloudflare_tunnel_config` + `cloudflare_record` resources for Headscale
- [ ] **Switch Terraform to S3 backend** — update `terraform/redstone/versions.tf` and `terraform/anton/versions.tf` to use an `s3` backend block pointed at MinIO on Storinator (`http://192.168.0.4:9000`, bucket `terraform-state`); add `minio_access_key` / `minio_secret_key` to `variables.tf` and `terraform.tfvars.example`
...
- [ ] **Configure static IPs on physical nodes** — run `ansible-playbook ansible/network.yml` for Anton and Redstone; set static IPs on Storinator and Gringotts via TrueNAS UI
...
- [ ] **NUT/UPS integration** — write an Ansible role `roles/nut/` that installs and configures NUT server on Orange Pi and NUT clients on Anton, Redstone, and Storinator; add it to `ansible/physical.yml` and the relevant VM playbooks
