...
- [ ] **Automate Vaultwarden secret storage via `bw` CLI** — after the one manual browser registration, write a script (or Ansible task in `ansible/roles/infra/`) that reads generated secrets from `/etc/homelab.env` on diglett-infra over SSH and uses `bw` CLI on the operator machine to create the following items in Vaultwarden:
  ...
  - `PostgreSQL` (Login, username = `postgres`, password = `POSTGRES_PASSWORD`, host = `192.168.0.23`)
