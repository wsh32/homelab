...
- `module.infra` -- `diglett-infra` VM (VM 201, `192.168.0.20`, 4 cores, 12GB): Traefik, step-ca, Infisical, Vaultwarden, Authentik, Litestream.
...
**`adguard/AdGuardHome.yaml`** -- Pre-seeded config. AdGuard detects a valid config on startup and skips the setup wizard entirely. Contains: bcrypt admin password hash (plaintext in Vaultwarden), upstream DNS (8.8.8.8 / 8.8.4.4), DNS rewrites (`*.wsh` → `100.64.1.20`, `*.home` A → `192.168.0.20`), and default blocklists.
...
**`docker-compose.yml`** -- Traefik (reverse proxy; ports 80/443), step-ca (local CA for TLS), Infisical (+ shared PostgreSQL + shared Redis), Vaultwarden, Authentik (+ shared PostgreSQL + shared Redis), and a Litestream sidecar that continuously streams the Vaultwarden SQLite WAL to Alakazam NFS. PostgreSQL runs a single instance with separate databases for Infisical and Authentik; same for Redis (separate logical DBs). Both run on local VM disk (not NFS) to avoid soft-mount corruption; PostgreSQL is backed up daily to Alakazam NFS via `pg_dumpall`.
...
**`traefik/dynamic/services-vm.yml`** -- File-provider routes for services running on other VMs. **Generated by Ansible** at deploy time from `network.yml` via `ansible/roles/infra/templates/services-vm.yml.j2`. Do not edit this file directly on the VM — changes are overwritten on the next `infra.yml` run. To add or change a service's routing, edit `network.yml`.
...
**`inventory/homelab.py`** -- Ansible inventory plugin. Reads `network.yml` and builds host groups from the `locations` hierarchy. Injects per-host vars: `nas_ip`, `traefik_ip`, `infisical_host`, `dns_fallback`, `nfs_mounts` (resolved NFS sources), `all_location_services` (flat list of all services in the location with VM IP), and `proxmox_nodes`. VMs with `ansible_managed: false` are excluded (e.g. diglett-haos).
