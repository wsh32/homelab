...
Add these to Infisical before doing anything else. All are machine-consumed secrets.

| Key | Description |
|-----|-------------|
| `PIA_USERNAME` | PIA VPN username |
| `PIA_PASSWORD` | PIA VPN password |
| `RADARR_API_KEY` | Generate: `python3 -c "import secrets; print(secrets.token_hex(16))"` |
| `SONARR_API_KEY` | Same |
| `PROWLARR_API_KEY` | Same |
| `BAZARR_API_KEY` | Same |
| `JELLYSEERR_API_KEY` | Same — also used as the Jellyseerr API token after setup |
| `JELLYFIN_ADMIN_PASSWORD` | Strong password for the Jellyfin admin account |

After adding, re-export on `machamp-services`:

```bash
ssh ubuntu@machamp-services.ts.home \
  "infisical export --env-file /etc/infisical.env --format dotenv > /etc/homelab.env"
```
