...
Add to `ansible/secrets.yml` before running `site.yml`:
...
`site.yml` (via per-service init roles) pushes these to Infisical and pre-seeds `sabnzbd.ini` with the Newshosting
...
corresponding API key to `secrets.yml`. No changes to playbooks needed.
...
| `BAZARR_API_KEY` | Generated by `arr-init` pre_tasks before Bazarr config is seeded |
| `SABNZBD_API_KEY` | Generated by `arr-init` pre_tasks before SABnzbd config is seeded |
| `JELLYFIN_ADMIN_PASSWORD` | Generated by `arr-init` pre_tasks before Jellyfin wizard runs |
...
delete `sabnzbd.ini` from `/mnt/nas/docker/sabnzbd/config/` and re-run `ansible-playbook ansible/site.yml` to
re-seed it.
...
[ ] site.yml arr-init tasks run a second time with 0 changed tasks
```
...
**arr-init fails on Prowlarr linking**: Prowlarr may still be indexing on first start.
...
healthy when arr-init ran. Re-run the playbook -- it will skip already-done steps and
...
`/mnt/nas/docker/sabnzbd/config/sabnzbd.ini` and re-run `ansible-playbook ansible/site.yml` to regenerate it
...
3. Run `ansible-playbook ansible/site.yml` -- the new indexer is added to Prowlarr and
