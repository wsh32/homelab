...
Only two secrets require manual entry — everything else is auto-generated
and injected by `arr-init.yml` on first run.
...
All other secrets are handled automatically:
...
After adding PIA credentials, re-export on `machamp-services` before running
`arr-init.yml` (or let `media.yml` do it automatically):

```bash
ssh ubuntu@machamp-services.ts.home \
  "source /etc/infisical.env && infisical export --format dotenv \
   --projectId \$INFISICAL_PROJECT_ID --env prod \
   --domain http://192.168.0.32:8080 > /etc/homelab.env"
```
