...
- Vaultwarden master password + client credentials (added after bootstrap step 10)

All service passwords are generated by Terraform (`random_password` / `random_id`) —
never manually typed. Terraform writes them into `.env` files on each VM via cloud-init
and into Vaultwarden via the `maxlaverse/bitwarden` provider. Services read `.env` at
startup — no runtime dependency on any secrets manager.

Developer API keys (Claude, Codex, GitHub) are NOT in `terraform.tfvars` — they are
entered directly into Infisical via its UI (bootstrap step 13).

Backed up as an encrypted note in Vaultwarden. Never committed to Git.
